#ifndef KAMRAT_RANK_SCORER_HPP
#define KAMRAT_RANK_SCORER_HPP
...
const std::vector<std::string> kScoreMethodName{"relat_sd", "ttest.padj", "snr", "lrc.f1", "nbc.f1", "svm.hingeloss", "user"};
enum SortModeCode
{
    kInc = 0,
    kIncAbs,
...
public:
    ...
    const void PrepareCountVect(const FeatureElem &feature_elem, const std::vector<double> &nf_vect, std::ifstream &idx_file,
...
private:
    ...
    arma::Mat<double> norm_count_vect_;             // normalized (and transformed) sample count vector, temporary
    std::vector<size_t> nbsmp_condi_;               // number of sample in each condition, temporary
};

#endif //KAMRAT_RANK_SCORER_HPP
