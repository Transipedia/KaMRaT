                    ...
                    const std::string &idx_path)
{
    ...
    std::string line;
    std::ofstream index_file(idx_path);
    ...
    for (size_t i(0); std::getline(kmer_count_instream, line); ++i)
    {
        ...
        float _;
        kmer_count_tab.AddIndexOnDsk(_, line, index_file);
    }
    index_file.close();
...
const void EvaluatePrintSeqElemFarthest(const std::string &tag,
                                        ...
                                        const std::string &idx_path)
{
    std::ifstream index_file(idx_path);
    ...
    {
        std::cout << tag << "\t" << MAX_DISTANCE << "\tNONE\tNONE" << std::endl;
    }
    else
    {
        ...
        kmer_count_tab.GetCountOnDsk(kmer1_count, iter1->second, index_file);
        kmer_count_tab.GetCountOnDsk(kmer2_count, iter2->second, index_file);
        std::cout << tag << "\t" << CalcDistance(kmer1_count, kmer2_count, eval_method) << "\t" << kmer1 << "\t" << kmer2 << std::endl;
    }
                                        ...
                                        const std::string &idx_path)
{
    ...
    {
        std::cout << tag << "\t" << dist << "\t" << kmer1 << "\t" << kmer2 << std::endl;
    }
...
int main(int argc, char **argv)
{
    ...
    std::string contig_fasta_path, eval_method, eval_mode, colname_list_path, idx_path("./counts.idx"), kmer_count_path;
    unsigned int k_len(31);
    bool stranded(true);

    ParseOptions(argc, argv, contig_fasta_path, eval_method, eval_mode, stranded, k_len, colname_list_path, idx_path, kmer_count_path);
    PrintRunInfo(contig_fasta_path, eval_method, eval_mode, stranded, k_len, colname_list_path, idx_path, kmer_count_path);

    std::cerr << "Option dealing finished, execution time: " << (float)(clock() - begin_time) / CLOCKS_PER_SEC << "s." << std::endl;
    inter_time = clock();

    CountTabByFields kmer_count_tab("onDsk");
    code2serial_t code2serial;
    ScanCountTable(kmer_count_tab, code2serial, kmer_count_path, colname_list_path, stranded, idx_path);
    ...
    for (const auto &ns_pair : tag_seq_dict)
    {
        ...
        {
            std::cout << tag << "\t" << MIN_DISTANCE << "\t" << seq << "\t" << seq << std::endl;
        }
        else if (seq.size() <= k_len) // if seq has only one k-mer but is not in k-mer count table or if it's shorter than 1 k-mer
        {
            std::cout << tag << "\t" << MAX_DISTANCE << "\tNONE\tNONE" << std::endl;
        }
        else if (eval_mode == "farthest") // if seq has multiple k-mers, and evaluation mode is "farthest"
        {
            EvaluatePrintSeqElemFarthest(tag, seq, eval_method, stranded, k_len, kmer_count_tab, code2serial, idx_path);
        }
        else if (eval_mode == "worstAdj")
        {
            EvaluatePrintSeqElemWorstAdj(tag, seq, eval_method, stranded, k_len, kmer_count_tab, code2serial, idx_path);
        }
    }
