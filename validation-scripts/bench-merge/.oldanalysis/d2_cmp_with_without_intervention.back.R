...
for (dpt in c(0.1, 0.2, 0.5, 1, 2, 5, 10)) {
    # KaMRaT none
    cat(dpt, "KaMRaT none", "\n")
    ctg.path <- paste0(work.dir, "kamrat_res_err-illumina5_2-2/depth_", dpt, "/ctg-seq.none.fa")
    align.path <- paste0(work.dir, "kamrat_res_err-illumina5_2-2/depth_", dpt, "/ctg-aligned.none.tsv")
    ...
    for (mode in c("pearson", "spearman", "mac")) {
        ...
        align.path <- paste0(work.dir, "kamrat_res_err-illumina5_2-2/depth_", dpt, "/ctg-aligned.", mode, "_0.2", ".tsv")
        ctg.fa <- readDNAStringSet(ctg.path)
        align.res <- read.table(align.path, header = TRUE, row.names = 1)
        if (!all(nchar(ctg.fa) == align.res$qlen[rownames(ctg.fa)])) {
            stop(paste(ctg.path, align.path, sep = "\n"))
    ...
    # rnaSPAdes
    for (mode in c("allreads", "allkmers-2-2")) {
        cat(dpt, "SPAdes", mode, "\n")
...
write.csv(stats.res, paste0(work.dir, "results/1_cmp_with_without_intervention.csv"),
          quote = FALSE)

pdf(paste0(work.dir, "results/1_cmp_with_without_intervention.pdf"),
    width=16, height=10)
