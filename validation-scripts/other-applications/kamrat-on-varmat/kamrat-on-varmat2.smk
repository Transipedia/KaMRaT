...
# nohup command:
#       nohup /home/haoliang.xue_ext/miniconda3/envs/kamrat-valid/bin/snakemake --snakefile kamrat-on-varmat2.smk --cluster "qsub -q lowprio -l nodes=1:ppn=1 -l mem=350G -l walltime=300:00:00 -m ea -M xue.haoliang@outlook.com" --jobs 6 -p --latency-wait 60 --rerun-incomplete >> workflow_kamrat-on-varmat2.txt &
...
rule all:
    input:
        ...
        expand(KAMRAT_DIR + "apply-on-varmat/{nsmp}-smp/kamrat-index-merge.{mergemthd}.tsv",
               nsmp = [170], mergemthd = ["pearson"]),
        expand(KAMRAT_DIR + "apply-on-varmat/{nsmp}-smp/kamrat-index-rank.{rankmthd}.tsv",
               nsmp = [170], rankmthd = ["snr"])
