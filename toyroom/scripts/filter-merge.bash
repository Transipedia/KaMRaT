...
mkdir -p output/filter-merge/ output/kamrat.idx
...
../bin/kamrat filter -idxdir output/kamrat.idx -design <(sed 's/normal/DOWN/g' data/sample-condition.toy.tsv | sed 's/tumor/UP/g') -upmin 5:5 -downmax 0:10 -outpath output/filter-merge/top-kmers.bin
...
../bin/kamrat merge -idxdir output/kamrat.idx -overlap 30-15 -with output/filter-merge/top-kmers.bin:min -outpath output/filter-merge/top-ctg-counts.tsv -withcounts mean
